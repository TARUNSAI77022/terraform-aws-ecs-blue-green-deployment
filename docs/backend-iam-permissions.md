    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:s3:::terraform-state-inti-ruchi",
                "arn:aws:s3:::terraform-state-inti-ruchi/*"
            ]
        },
        {
...
1. Ensure the S3 bucket `terraform-state-inti-ruchi` actually exists in `ap-south-1` and has **Bucket Versioning** enabled.
2. Ensure the DynamoDB table `terraform-locks` exists in `ap-south-1` and has a Partition Key named `LockID` (String type).
